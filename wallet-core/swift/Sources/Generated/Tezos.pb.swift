...
public struct TW_Tezos_Proto_Operation {
  ...
  public var delegationOperationData: TW_Tezos_Proto_DelegationOperationData {
    get {
      if case .delegationOperationData(let v)? = _storage._operationData {return v}
      return TW_Tezos_Proto_DelegationOperationData()
    }
    set {_uniqueStorage()._operationData = .delegationOperationData(newValue)}
  }
    ...
    try withExtendedLifetime(_storage) { (_storage: _StorageClass) in
      while let fieldNumber = try decoder.nextFieldNumber() {
        ...
        case 11:
          ...
          try decoder.decodeSingularMessageField(value: &v)
          if let v = v {_storage._operationData = .delegationOperationData(v)}
        default: break
        }
      }
    }
